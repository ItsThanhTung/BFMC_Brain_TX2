...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    stride, names, pt, jit, onnx, engine =32, model.names, model.pt, model.jit, model.onnx, model.engine
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    line = (cls, *xywh, conf)
                    json_arr.append(str(('%g ' * len(line)).rstrip() % line))
            ...
            # send image
            # success, encoded_image=cv2.imencode('.png',im0)
            # im_bytes = encoded_image.tobytes()
            # im_b64=base64.b64encode(im_bytes).decode("utf8")

            # headers = {'Content-type': 'application/json', 'Accept': 'text/plain'}
  
            # payload = json.dumps({"image": im_b64, "od": json_arr,"time": datetime.now().strftime("%d/%m/%Y %H:%M:%S")})
